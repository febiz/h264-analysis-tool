...
int main(int argc, char **argv)
{
	...
	do
	{
		...
		{
			// EXT ----------------------------------------------------------------
			unsigned int dpb_used = p_Dec->p_Vid->p_Dpb_layer[p_Dec->p_Vid->last_dec_view_id]->used_size;
			if (dpb_used > 0) {
				// newly decoded picture is stored at the end of fs buffer
				StorablePicture *spic = p_Dec->p_Vid->p_Dpb_layer[p_Dec->p_Vid->last_dec_view_id]->fs[dpb_used-1]->frame;
				int frame_number = spic->poc/p_Dec->p_Vid->poc_gap;
				visualizer.put(spic, p_Dec->p_Vid->mb_data, frame_number);
			}
			// END EXT ------------------------------------------------------------
